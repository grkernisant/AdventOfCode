  ...
  public function __construct(array $lines)
  {
    ...
    foreach($lines as $line) {
      if ($matches = preg_split(static::SPACE_REGEX, trim($line))) {
        $matches = array_map(fn($x) => $x, $matches);
...
class Equation
{
  ...
  public function __construct(public string $result, public array $numbers, public bool $valid = false) {}
...
class EquationMember
{
  ...
  public function __construct(public string $number, public string $operator, public ?string $subtotal = null)
  {
    $this->children = array();
  }
      ...
      switch ($this->operator) {
        case static::OPERATOR_ADD:
          $n->subtotal = $this->subtotal !== null
            ? bcadd((string) $this->subtotal, (string) $n->number)
            : (string) $n->number;
        break;

        case static::OPERATOR_MULTIPLY:
          $n->subtotal = $this->subtotal !== null
            ? bcmul((string) $this->subtotal, (string) $n->number)
            : (string) $n->number;
        break;

        case static::OPERATOR_CONCAT:
          $n->subtotal = $this->subtotal !== null
            ? (string) $this->subtotal . (string) $n->number
            : (string) $n->number;
        break;
      }
  ...
  public function find(string $result): bool
  {
